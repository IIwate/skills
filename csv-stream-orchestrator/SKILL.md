...
description: 将需求转为并行探索、计划评审、CSV 子任务分发、流式验收和失败回流重分发的执行流程。仅当用户消息以 `$csv-stream-orchestrator` 开头时使用；任何不含此前缀的请求都不要触发本技能。用于手动触发后先等待用户输入需求，再按模板生成计划、CSV 和 worker 指令。
---
...
9. 按“批次启动 -> 收敛验收 -> close -> cleanup 验证”的顺序循环，直到所有批次（含回流批次）处理完毕。
10. 全部任务闭环后统一收口：冲突处理、全量验证、交付说明。
